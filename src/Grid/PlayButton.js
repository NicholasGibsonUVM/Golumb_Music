...
class PlayButton extends React.Component {
    ...
    playSynth = () => {
        ...
        for (let i = 0; i < this.props.activeNotes.length; i++) {
            ...
            synth.triggerAttackRelease(this.numberToNote(this.props.activeNotes[i].y + noteShift), "8n", now + i);
        }
    }
    ...
    render() {
        return (
            <div>
                <button id="playButton" onClick={this.playSynth}>Play Song</button>
            </div>)
    }
}